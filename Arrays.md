...
**Пример с параметрами `index` & `array`:**
...
**Совет:** если одна логика нужна в нескольких методах — выноси колбэк:
...
## `.forEach(callback)` — перебор массива

**Описание:** вызывает функцию для каждого элемента массива

**Параметры:**

- `callback(value, index, array)` — выполняется на каждой итерации
...
skills.forEach(logValues); // без скобок!
```
...
## `Array.isArray(value)` — проверка типа
...
## `Array.of(...values)` — создание из аргументов
...
## `Array.from(iterable)` — создание из итерируемого
...
**Важно:** rest всегда должен стоять последним:
